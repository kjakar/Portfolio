...
#======================================== lab02 item (uncompleted) ===================================================#
    ...
    def crossProduct(self, other):
        ...
        if isinstance(other, VectorN) and self.__mDim == other.__mDim  and self.__mDim >= 3:
            cx = (self.__mData[1] * other.__mData[2]) - (self.__mData[2] * other.__mData[1])
            cy = (self.__mData[2] * other.__mData[0]) - (self.__mData[0] * other.__mData[2])
            cz = (self.__mData[0] * other.__mData[1]) - (self.__mData[1] * other.__mData[0])
            c = VectorN(cx,cy,cz)
            return c
        else:
            raise ValueError("You can only fine the cross product of two VectorN's with at least 3 dimensions")
