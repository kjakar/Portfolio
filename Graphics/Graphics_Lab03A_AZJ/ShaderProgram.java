...
public class ShaderProgram {
	...
	public boolean setModelMatrix(Matrix4 m) {
		...
		return this.setMatrix(flat_data, "ModelMatrix");
	}
	
	public boolean setViewMatrix(Matrix4 m) {
		float[] flat_data = new float[16];
		int k = 0;
		for (int i = 0; i < 4; ++i) {
			for (int j = 0; j < 4; ++j) {
		...
		return this.setMatrix(flat_data, "ViewMatrix");
	}
	
	public boolean setProjectionMatrix(Matrix4 m) {
		float[] flat_data = new float[16];
		int k = 0;
		for (int i = 0; i < 4; ++i) {
			for (int j = 0; j < 4; ++j) {
		...
		return this.setMatrix(flat_data, "ProjectionMatrix");
	}
	
	public boolean setVector4(Vector4 v, String name) {
		int vectorLocation = glGetUniformLocation(_shaderProgram, name);
		
		if (vectorLocation == -1)
			return false;
		
		glUniform4fv(vectorLocation, v.data);
		return true;
	}
	
	public boolean setMatrix(float[] value, String name) {
		int matrixLocation = glGetUniformLocation(_shaderProgram, name);
		
		if (matrixLocation == -1)
			return false;
		
		glUniformMatrix4fv(matrixLocation, true, value);
		return true;
	}
